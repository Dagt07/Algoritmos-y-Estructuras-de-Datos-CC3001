# -*- coding: utf-8 -*-
"""Tarea 5 AED David Garcia

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/13AtjQH81q_xlIKwiFU-votgNqX0Gs5LX

# CC3001 Otoño 2023 Tarea 5 David Garcia
...
class Nodoi:
    ...
    def insert(self,x):
      assert x!=self.info
      cont_nuevo = self.contador + 1
      if x<self.info:
          return Nodoi(self.izq.insert(x), self.info, cont_nuevo, self.der)
      else:
          return Nodoi(self.izq, self.info, cont_nuevo, self.der.insert(x))
    ...
    def find(self,k):
      if k > self.contador: #K esta fuera de rango, se retorna None por enunciado
        return None

      else: #Caso de puntero al k-esimo

        #Nos encontramos en la raiz del nodo actual, revisamos el cont del hijo izq
        cont_hijoizq = self.izq.contador
        if k == self.izq.contador + 1:
            return self
        elif cont_hijoizq >= k: #Está en el subarbol izq
            return self.izq.find(k)
        else: #Caso en que está en subarbol der
          #Redefinimos k, aqui no va a ser el k original sino: contador del nodo actual - k
          k = self.contador - k
          cont_hijoder = self.der.contador
          if k == cont_hijoder + 1:
            return self
          else:
...
class Nodoe:
    ...
    def insert(self,x):
      cont_nuevo = 1
      return Nodoi(Nodoe(0), x, cont_nuevo, Nodoe(0))
